 ...
 */
metadata {
	definition (name: "Aeotec HEM Gen5", namespace: "rapportnetworks", author: "Artur Draga") {
...
def updated() {
	...
	parameterMap().each {
		if(settings."$it.key" != null || it.num in (101..103) ) {
			...
			else if (it.num == 3 || it.num == 13) { value = (settings."$it.key" == 'true') ? 1 as Integer : 0 as Integer }
