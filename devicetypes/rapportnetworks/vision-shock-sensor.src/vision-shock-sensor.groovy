...
def parse(String description) {
    ...
    */
    ...
    logTrace "Raw message: $description"
...
def zwaveEvent(physicalgraph.zwave.commands.securityv1.SecurityMessageEncapsulation cmd) {
    ...
    def result = []
    if (encapCmd) {
        logTrace "Encapsulated mesage received."
...
private getCommandClassVersions() {
    [
            ...
            0x71: 3,  // Alarm v1 or Notification (v4) - ??V4
            0x72: 2,  // ManufacturerSpecific
            0x73: 1,  // Powerlevel (Model 0308)
            0x80: 1,  // Battery
            0x84: 2,  // WakeUp
            0x85: 2,  // Association
            0x86: 1,  // Version (v2) - ??V2
...
def zwaveEvent(physicalgraph.zwave.commands.notificationv3.NotificationReport cmd) {
    ...
    def result = []
    if (cmd.notificationType == 7) { // Burglar
        if (cmd.event == 2 || cmd.v1AlarmType == 2) { // Vibration
            // createPrimaryEventMaps(cmd.v1AlarmLevel == 0XFF)?. each {
            // result << createEvent(it)
            // }
        }
        else if (cmd.event == 3 || cmd.v1AlarmType == 3) { // Tamper
